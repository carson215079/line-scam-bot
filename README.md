...
| 🕷️ **自動爬蟲** | 每 3 小時從台灣媒體 RSS（中央社/自由/ETtoday）抓取，關鍵字過濾詐騙新聞 |
| 🤖 **AI 摘要** | 抓取文章內文，AI 讀全文生成易懂的標題與摘要 |
| 🔁 **推播去重** | 每篇文章只推播一次，不重複推送已發過的新聞 |
...
│
├── crawler/
│   └── base.py                  # 台灣媒體 RSS 爬蟲（關鍵字過濾）+ 內文抓取
│
├── scheduler/
│   └── daily_job.py             # 定時爬蟲 + 每日推播（去重）+ 每日清理（對話/容量）
│
└── scripts/
    ├── remigrate_articles.py    # 舊文章重整（抓內文 + 重寫摘要）
    ├── fix_urls.py              # 批次解碼 Google 連結（歷史遺留資料用）
...
### 爬蟲（每 3 小時）
```
台灣媒體 RSS（中央社/自由/ETtoday）→ 關鍵字過濾詐騙新聞
→ 確認是新文章（避免重複耗費 API）→ 抓取內文 → AI 讀全文寫標題+摘要
→ 存入 DB（URL 唯一，自動擋重複；來源即真實網址，免解碼）
```
...
### 每日排程
```
09:00  推播最新 5 篇「未推播過」文章（每篇一則訊息，不重複）
